...
int options::parse(int argc, char** argv) {
    ...
    for (i = 2; i < argc; ++i) {
        ...
        if (!strcmp(arg, "--tree")) bit = 1;
        else if (!strcmp(arg, "--format")) bit = 2;
        else if (!strcmp(arg, "--input")) bit = 4;
        else if (!strcmp(arg, "--hex")) bit = 8;
        else if (!strcmp(arg, "--max-input-size")) bit = 16;
        else if (!strcmp(arg, "--max-depth")) bit = 32;
        else if (!strcmp(arg, "--max-elements")) bit = 64;
        else if (!strcmp(arg, "--pretty")) bit = 128;
        else if (!strcmp(arg, "--describe")) bit = 256;
        else if (!strcmp(arg, "--force-color")) bit = 512;
        else if (!strcmp(arg, "--no-color")) bit = 1024;
        else if (!strcmp(arg, "--profile")) bit = 2048;
        else if (!strcmp(arg, "--input-encoding")) bit = 4096;
        else if (!strcmp(arg, "--pdol")) bit = 8192;
        else return fail(2, "unknown option; use --help");
        if (seen & bit) return fail(2, "duplicate option");
        seen |= bit;
        if (bit == 1) { tree = 1; continue; }
        if (bit == 8192) { pdol = 1; continue; }
        if (bit == 128) { pretty = tree = 1; continue; }
        if (bit == 256) { describe = 1; continue; }
        if (bit == 512 || bit == 1024) { color = bit == 512 ? 1 : -1; continue; }
        if (++i == argc) return fail(2, "missing option value");
        if (bit == 2) format = argv[i];
        else if (bit == 4) input = argv[i];
        else if (bit == 8) hex = argv[i];
        else if (bit == 2048) profile = argv[i];
        else if (bit == 4096) {
...
void options::usage() {
    std::puts("Usage: opentlv dump|validate --format NAME (--input PATH|- | --hex BYTES) [OPTIONS]\n"
