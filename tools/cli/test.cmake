...
check(2 "unknown shell" completion unknown)
check(2 "exactly one shell" completion bash extra)
check(0 "^# bash completion for otlv" completion bash)
check(0 "^#compdef otlv" completion zsh)
check(0 "^# fish completion for otlv" completion fish)
check(0 "^# PowerShell completion for otlv" completion powershell)
check(0 "--format" completion bash)
execute_process(COMMAND "${CLI}" completion bash RESULT_VARIABLE result
    OUTPUT_VARIABLE output ERROR_VARIABLE error ENCODING UTF-8)
if(NOT result EQUAL 0 OR NOT error STREQUAL "")
    message(FATAL_ERROR "otlv completion bash failed: ${result} ${error}")
endif()
if(HAS_EMV)
    if(NOT output MATCHES "query tag tags")
        message(FATAL_ERROR "completion bash: missing tag/tags with EMV enabled: ${output}")
    endif()
    if(NOT output MATCHES "emv-context")
        message(FATAL_ERROR "completion bash: missing --emv-context with EMV enabled: ${output}")
    endif()
else()
    if(output MATCHES "query tag tags")
        message(FATAL_ERROR "completion bash: tag/tags should be omitted without EMV: ${output}")
    endif()
    if(output MATCHES "emv-context")
        message(FATAL_ERROR "completion bash: --emv-context should be omitted without EMV: ${output}")
    endif()
endif()
if(HAS_FIXED)
    if(NOT output MATCHES "fixed-byte-order")
        message(FATAL_ERROR "completion bash: missing --fixed-byte-order with fixed enabled: ${output}")
    endif()
else()
    if(output MATCHES "fixed-byte-order")
        message(FATAL_ERROR "completion bash: --fixed-byte-order should be omitted without fixed: ${output}")
    endif()
endif()
...
if(HAS_FIXED)
    round_trip(fixed 01 AABB)
    check(1 "cannot encode element 0: " encode --format fixed --tag 0102 --value AA)
    check(0 "^010203AABBCC\n$" encode --format fixed --fixed-tag-size 2 --fixed-length-size 1
        --tag 0102 --value AABBCC)
    check(0 "offset=0 tag=0102 length=3 value=AABBCC" dump --format fixed --fixed-tag-size 2
