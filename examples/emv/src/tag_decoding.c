...
static int build_record(uint8_t* content, size_t capacity, size_t* content_size) {
    ...
    /* Application Expiration Date (5F24): YYMMDD BCD. */
    {
        ...
        if (encode_child(tlv_emv_find(TLV_EMV_CONTEXT_BASE, &tlv_emv_tag_application_expiration_date)->codec,
    ...
    /* AIP (82): SDA and CDA supported. */
    {
        ...
        if (encode_child(tlv_emv_find(TLV_EMV_CONTEXT_BASE, &tlv_emv_tag_aip)->codec,
                         &aip, sizeof(aip), scratch, sizeof(scratch), &size))
            return 1;
    }
    ...
    /* Cryptogram Information Data (9F27): ARQC requested. */
    {
        ...
        if (encode_child(tlv_emv_find(TLV_EMV_CONTEXT_BASE, &tlv_emv_tag_cryptogram_information_data)->codec,
     ...
     * dictionary's required three, to demonstrate a length mismatch. */
    {
        ...
        CHECK(tlv_writer_write(&writer, tlv_emv_tag_cvm_results, bad_cvm_results, sizeof(bad_cvm_results)));
    }
...
typedef struct {
    ...
    int pan_seen; char pan[32];
    int expiry_seen; tlv_emv_date_t expiry;
    int aip_seen; uint64_t aip;
    int afl_seen; uint8_t afl[4]; size_t afl_length;
    int amount_seen; uint64_t amount;
    int cryptogram_seen; tlv_emv_cryptogram_info_t cryptogram;
    int account_seen; tlv_emv_account_type_t account;
} record_t;
...
static tlv_visit_result_t decode_field(const tlv_view_t* view, void* context) {
    ...
    case TLV_EMV_VALUE_DIGITS: {
        ...
        if (rc != TLV_CODEC_OK) { printf(" -> decode error: %s\n", tlv_codec_strerror(rc)); ++record->errors; break; }
        printf(" = %s\n", record->pan);
        record->pan_seen = 1;
        break;
    }
    case TLV_EMV_VALUE_DATE: {
        ...
        printf(" = 20%02u-%02u-%02u (century assumed by the caller)\n",
               (unsigned)record->expiry.year, (unsigned)record->expiry.month, (unsigned)record->expiry.day);
    ...
    case TLV_EMV_VALUE_CRYPTOGRAM: {
        ...
        tlv_codec_result_t rc = tlv_codec_decode(def->codec, view->value.data, length,
                                                 &record->cryptogram, sizeof(record->cryptogram));
        if (rc != TLV_CODEC_OK) { printf(" -> decode error: %s\n", tlv_codec_strerror(rc)); ++record->errors; break; }
    ...
    case TLV_EMV_VALUE_ACCOUNT: {
        ...
        printf(" = %s\n", record->account == TLV_EMV_ACCOUNT_CREDIT ? "credit" :
               ...
               record->account == TLV_EMV_ACCOUNT_CHEQUE_DEBIT ? "cheque/debit" : "default");
        record->account_seen = 1;
        break;
    }
    case TLV_EMV_VALUE_BYTES: {
        size_t i;
        if (length > sizeof(record->afl)) { printf(" -> value too large for this example\n"); ++record->errors; break; }
...
int main(void) {
    ...
    CHECK(tlv_walk(outer.value.data, wire_content_size, &tlv_reader_format_ber, decode_field, &record));
    ...
    EXPECT(record.pan_seen && strcmp(record.pan, "4111111111111111") == 0, "PAN decoded incorrectly");
    EXPECT(record.expiry_seen && record.expiry.year == 27 && record.expiry.month == 12 &&
          record.expiry.day == 31, "expiration date decoded incorrectly");
    EXPECT(record.aip_seen &&
          record.aip == (TLV_EMV_AIP_SDA_SUPPORTED | TLV_EMV_AIP_CDA_SUPPORTED), "AIP decoded incorrectly");
    {
        static const uint8_t expected_afl[] = {0x08, 0x01, 0x01, 0x00};
        EXPECT(record.afl_seen && record.afl_length == sizeof(expected_afl) &&
              memcmp(record.afl, expected_afl, sizeof(expected_afl)) == 0, "AFL decoded incorrectly");
    }
    ...
    EXPECT(record.cryptogram_seen && record.cryptogram.type == TLV_EMV_CRYPTOGRAM_ARQC &&
          record.cryptogram.flags == 0x15, "cryptogram information decoded incorrectly");
    EXPECT(record.account_seen && record.account == TLV_EMV_ACCOUNT_CREDIT, "account type decoded incorrectly");
