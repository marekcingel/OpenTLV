...
- Fix the clang-tidy findings the new check surfaced across `tlv` and `tlv++` (unparenthesized macro parameters, a redundant cast, non-uppercase integer literal suffixes, several narrow-then-widened multiplications made explicit, two uninitialized locals left garbage on an unverified cross-file success contract, a public schema-dictionary lookup missing a defense-in-depth bounds check, a redundant header include, and a template polyfill's move constructor missing a conditional `noexcept`) so the check starts green; a few remaining Clang Static Analyzer findings are genuine false positives (through indirect function-pointer calls or a recursive arena-write chain the analyzer can't fully fold) and are suppressed with a `NOLINT`/`NOLINTBEGIN`-`NOLINTEND` comment explaining why. (#120)
