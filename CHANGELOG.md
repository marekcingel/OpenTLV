...
- Add a `fuzz_dol` libFuzzer target (`tests/fuzz/dol.c`, built when `OPENTLV_PROFILE_EMV` is on) exercising `tlv_dol_read` and both a `tlv_dol_write` size query and a full write against a deterministic `resolve` callback, plus both DOL resource limits. Add a seed corpus under `tests/fuzz/corpus/dol/` and wire the target into the Clang CI fuzzing job and [docs/fuzzing.md](docs/fuzzing.md). (#56)
...
- Add a `fuzz_der_schema` libFuzzer target (`tests/fuzz/der_schema.c`, built when `OPENTLV_FORMAT_DER` is on) exercising `tlv_der_schema_read` against a fixed representative schema covering implicit/explicit tagging, a DEFAULT component, SET, SET OF and CHOICE, plus all five schema resource limits. Add a seed corpus under `tests/fuzz/corpus/der_schema/` and wire the target into the Clang CI fuzzing job and [docs/fuzzing.md](docs/fuzzing.md). (#63)
- Document the schema-aware DER layer's supported ASN.1 subset and strict-validation guarantees in a new "Schema-aware validation and encoding" section of [docs/profiles/der/README.md](docs/profiles/der/README.md), cross-linked from [docs/schemas.md](docs/schemas.md) to distinguish it from the generic, format-agnostic `tlv_structure_schema_t`. (#63)
