...
- Add a `cppcheck` job to [static-analysis.yml](.github/workflows/static-analysis.yml), configuring the project with CMake + Ninja in Debug mode and building the `cppcheck` target, alongside the existing pre-commit formatting job; the workflow is structured so clang-tidy can follow as another job later. (#119)
