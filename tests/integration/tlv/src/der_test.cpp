...
TEST(Integration_Der, LimitsAndWriterValidation) {
    ...
    for (int limit = 0; limit < 4; ++limit) {
        ...
        EXPECT_EQ(TLV_ERR_LIMIT, tlv_der_write(nullptr, 0, (tlv_tag_t{{0x30}, 1}), data + 2, 4,
    ...
    EXPECT_EQ(TLV_ERR_INVALID_LENGTH,
              tlv_der_write(output, sizeof(output), (tlv_tag_t{{0x30}, 1}), invalid,
                            sizeof(invalid), nullptr, &written, &offset));
    EXPECT_EQ(3u, offset);
    EXPECT_EQ(99u, written);
    for (auto byte : output) EXPECT_EQ(0xEE, byte);
    EXPECT_EQ(TLV_ERR_BUFFER_TOO_SHORT, tlv_der_write(output, 1, (tlv_tag_t{{4}, 1}), nullptr, 0,
...
TEST(Integration_Der, DeepNestingUsesBoundedTraversal) {
    ...
    for (size_t depth = 1; depth <= TLV_DER_MAX_DEPTH + 1; ++depth) {
        size_t size;
        ASSERT_EQ(TLV_OK, tlv_encoded_size((tlv_tag_t{{0xA0}, 1}), data.size(),
                                           &tlv_writer_format_der, &size));
