...
struct Segment { tlv_tag_t tag; tlv_value_t value; };
...
TEST(Integration_Cer, NestedIndefiniteContainersPostorderVisitOrder) {
...
TEST(Integration_Cer, PrimitiveContentContainingEocAndIndefiniteMarkerDoesNotAffectNesting) {
...
TEST(Integration_Cer, EmptyIndefiniteContainer) {
...
TEST(Integration_Cer, MissingTruncatedAndUnexpectedEoc) {
    ...
    for (const auto& item : cases) {
        ...
        size_t offset = 99;
        EXPECT_EQ(item.error, tlv_cer_walk(item.bytes.data(), item.bytes.size(), nullptr,
                                          nullptr, nullptr, &offset));
        EXPECT_EQ(item.offset, offset);
    }
}

TEST(Integration_Cer, SingleElementReadLeavesFollowingElementUnconsumed) {
...
TEST(Integration_Cer, DepthLimitBoundary) {
...
TEST(Integration_Cer, ConstructedFormRejectedStructurallyEvenWithoutStrict) {
...
TEST(Integration_Cer, StrictReadRejectsNoncanonicalContentButNonStrictAccepts) {
    ...
    ASSERT_EQ(TLV_OK, tlv_cer_read(data, sizeof(data), nullptr, &view, &consumed, nullptr));
    EXPECT_EQ(sizeof(data), consumed);
    EXPECT_EQ(TLV_ERR_INVALID_VALUE, tlv_cer_read_strict(data, sizeof(data), nullptr, &view, &consumed, &offset));
    EXPECT_EQ(7u, offset);
}

TEST(Integration_Cer, StrictRejectsUnsupportedUniversalTypeIncludingConstructed) {
    ...
    EXPECT_EQ(TLV_ERR_UNSUPPORTED_TYPE,
              tlv_cer_read_strict(primitive, sizeof(primitive), nullptr, &view, &consumed, &offset));
    ...
    seg1[0] = 0x14; seg1[1] = 0x82; seg1[2] = 0x03; seg1[3] = 0xE8; /* tag, len=1000 */
    constructed.insert(constructed.end(), seg1.begin(), seg1.end());
    constructed.push_back(0x14); constructed.push_back(1); constructed.push_back('y');
    constructed.push_back(0); constructed.push_back(0);
    EXPECT_EQ(TLV_ERR_UNSUPPORTED_TYPE,
              tlv_cer_read_strict(constructed.data(), constructed.size(), nullptr, &view, &consumed, &offset));
}

TEST(Integration_Cer, WriteConstructedProducesCanonicalIndefiniteFraming) {
...
TEST(Integration_Cer, WriteRejectsOversizedPrimitiveForSegmentableTypeEvenNonStrict) {
    ...
    EXPECT_EQ(TLV_ERR_INVALID_LENGTH,
              tlv_cer_write(nullptr, 0, (tlv_tag_t{{0x04}, 1}), value.data(), value.size(),
                           nullptr, &written, &offset));
    EXPECT_EQ(99u, written);
}

TEST(Integration_Cer, FailedWritePreservesOutputAndSize) {
    const uint8_t children[] = {0x02, 1, 5};
    uint8_t output[8];
    std::memset(output, 0xEE, sizeof(output));
    size_t written = 99, offset = 99;
    EXPECT_EQ(TLV_ERR_BUFFER_TOO_SHORT,
              tlv_cer_write(output, 1, (tlv_tag_t{{0x30}, 1}), children, sizeof(children),
                           nullptr, &written, &offset));
    EXPECT_EQ(99u, written);
    for (auto byte : output) EXPECT_EQ(0xEE, byte);
}

TEST(Integration_Cer, WriteSegmentedStringSingleAndMultiSegment) {
    /* Content at or under the threshold: a single canonical primitive. */
    {
        std::vector<uint8_t> content(1000, 'a');
        uint8_t output[1010];
        size_t required = 0, written = 0;
        ASSERT_EQ(TLV_OK, tlv_cer_write_segmented_string(nullptr, 0, (tlv_tag_t{{0x04}, 1}),
                          content.data(), content.size(), nullptr, &required, nullptr));
        ASSERT_EQ(TLV_OK, tlv_cer_write_segmented_string(output, sizeof(output), (tlv_tag_t{{0x04}, 1}),
                          content.data(), content.size(), nullptr, &written, nullptr));
        EXPECT_EQ(required, written);
    ...
    /* Content over the threshold: constructed, segmented, zero-copy segments. */
    {
        ...
        ASSERT_EQ(TLV_OK, tlv_walk(view.value.data,
                  static_cast<size_t>(view.value.length), &tlv_reader_format_cer, collect_segment, &segments));
