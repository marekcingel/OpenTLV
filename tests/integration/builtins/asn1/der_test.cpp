...
TEST(Integration_Tlv_Der, TagClassesNumbersAndForms) {
    for (auto cls :
        ...
        for (uint64_t number : {UINT64_C(4), UINT64_C(30), UINT64_C(31), UINT64_C(127),
            ...
            for (int constructed : {0, 1}) {
                ...
                const bool invalid = digits > TLV_ASN1_TAG_MAX_SIZE ||
                                     (cls == TLV_ASN1_UNIVERSAL && number <= 36 && constructed);
                ASSERT_EQ(digits > TLV_ASN1_TAG_MAX_SIZE ? TLV_ERR_INVALID_TAG_SIZE
...
TEST(Integration_Tlv_Der, CanonicalLengthBytesAndRoundTrip) {
    ...
    for (size_t length : {0u, 1u, 127u, 128u, 255u, 256u, 65535u, 65536u}) {
        ...
        ASSERT_EQ(TLV_OK, tlv_der_write(nullptr, 0, tag, value.data(), length, nullptr, &required,
                                        nullptr));
...
TEST(Integration_Tlv_Der, InvalidFieldsHaveOffsetsAndPreserveOutputs) {
    struct Case {
...
TEST(Integration_Tlv_Der, LimitsAndWriterValidation) {
    ...
    for (int limit = 0; limit < 4; ++limit) {
        ...
        switch (limit) {
            case 0:
                ...
                break;
            case 1:
                restricted.max_input_size = 5;
                expected = 0;
                break;
            case 2:
                restricted.max_value_size = 3;
                expected = 1;
                break;
            default:
                restricted.max_elements = 2;
                expected = 4;
                break;
        }
    ...
    EXPECT_EQ(TLV_ERR_BUFFER_TOO_SHORT,
              tlv_der_write(output, 1, (TLV_TAG(4)), nullptr, 0, nullptr, &written, &offset));
    for (auto byte : output) EXPECT_EQ(0xEE, byte);
}

TEST(Integration_Tlv_Der, DeepNestingUsesBoundedTraversal) {
...
TEST(Integration_Tlv_Der, BerCompatibilityAndGenericFormat) {
