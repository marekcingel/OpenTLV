...
TEST(Integration_Tlv_Ber, IndefiniteReferenceEncodingsAndRoundTrip) {
    ...
    for (const auto& value : values) {
        ...
        ASSERT_EQ(TLV_OK, tlv_ber_write_indefinite(output.data(), output.size(), tag, value.data(),
                                                   value.size(), &written));
...
TEST(Integration_Tlv_Ber, IndefiniteMalformedInputIsAtomic) {
    struct Case {
...
TEST(Integration_Tlv_Ber, IndefiniteNestingLimitIncludesDefiniteScopes) {
