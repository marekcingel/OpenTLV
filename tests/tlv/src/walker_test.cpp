...
TEST(Walker, EmptyInputSucceedsAndNullContextIsAllowed) {
    const uint8_t data[] = {1, 0};
    Visits visits;
    EXPECT_EQ(TLV_OK, tlv_walk(nullptr, 0, &tlv_reader_format_fixed_1byte, collect, &visits));
    EXPECT_EQ(TLV_OK, tlv_walk(data, 0, &tlv_reader_format_fixed_1byte, collect, &visits));
    EXPECT_EQ(0u, visits.count);
    EXPECT_EQ(TLV_OK, tlv_walk(data, sizeof(data), &tlv_reader_format_fixed_1byte,
...
TEST(Walker, RejectsInvalidArgumentsEvenForEmptyInput) {
    const uint8_t data[] = {1, 0};
    Visits visits;
    EXPECT_EQ(TLV_ERR_NULL_ARG, tlv_walk(nullptr, 1, &tlv_reader_format_fixed_1byte, collect, &visits));
    for (size_t size : {size_t(0), sizeof(data)}) {
        EXPECT_EQ(TLV_ERR_NULL_ARG, tlv_walk(data, size, nullptr, collect, &visits));
        EXPECT_EQ(TLV_ERR_NULL_ARG, tlv_walk(data, size, &tlv_reader_format_fixed_1byte, nullptr, &visits));
        auto format = tlv_reader_format_fixed_1byte;
