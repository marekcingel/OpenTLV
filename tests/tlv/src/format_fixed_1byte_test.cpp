...
TEST(Fixed1Byte, ExampleWireBytes) {
    ...
    tlv_writer_t writer;
    ASSERT_EQ(TLV_OK, tlv_writer_init(&writer, data, sizeof(data), &tlv_format_fixed_1byte));
...
TEST(Fixed1Byte, EveryTagAndLengthRoundTrip) {
    ...
    for (size_t byte = 0; byte <= 255; ++byte) {
        ...
        tlv_reader_t reader;
        ASSERT_EQ(TLV_OK, tlv_reader_init(&reader, data, writer.pos, &tlv_format_fixed_1byte));
        tlv_view_t entry{};
...
TEST(Fixed1Byte, TruncationPreservesReaderAndOutput) {
    ...
    for (size_t size = 0; size < sizeof(data); ++size) {
        tlv_reader_t reader;
        ASSERT_EQ(TLV_OK, tlv_reader_init(&reader, data, size, &tlv_format_fixed_1byte));
...
TEST(Fixed1Byte, InvalidWritesPreserveBufferAndPosition) {
    ...
    for (size_t capacity = 0; capacity < 5; ++capacity) {
        ASSERT_EQ(TLV_OK, tlv_writer_init(&writer, data, capacity, &tlv_format_fixed_1byte));
...
TEST(Fixed1Byte, CallbacksRejectMissingBytes) {
    const auto& format = tlv_format_fixed_1byte;
    ...
    ASSERT_EQ(TLV_OK, format.write_tag(nullptr, nullptr, 0, &tag, &used));
    EXPECT_EQ(1u, used);
    uint8_t data = 0xEE;
    EXPECT_EQ(TLV_ERR_BUFFER_TOO_SHORT, format.write_tag(nullptr, &data, 0, &tag, &used));
    EXPECT_EQ(TLV_ERR_BUFFER_TOO_SHORT, format.write_length(nullptr, &data, 0, 255, &used));
    EXPECT_EQ(0xEE, data);
}
