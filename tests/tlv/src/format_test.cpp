...
TEST(Format, TruncationPreservesReaderStateAndOutput) {
...
TEST(Format, WriterPreflightDoesNotModifyBuffer) {
    uint8_t data[5];
...
TEST(Format, RequiredCallbacksAreValidatedPerDirection) {
    tlv_reader_t reader;
    tlv_writer_t writer;
