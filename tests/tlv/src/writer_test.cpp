...
TEST(Writer, SizesWireEncodingAndRoundTripsAtLengthBoundaries) {
    for (const auto* format : {&tlv_format_fixed_1byte, &tlv_format_default}) {
        for (size_t length : {0u, 1u, 127u, 128u, 255u, 256u, 65535u}) {
            if (format == &tlv_format_fixed_1byte && length > 255) continue;
            SCOPED_TRACE(length);
            ...
            const size_t length_bytes = format == &tlv_format_fixed_1byte || length < 128
                ? 1 : (length <= 255 ? 2 : 3);
...
TEST(Writer, EveryInsufficientCapacityPreservesBufferAndOutput) {
    ...
    for (size_t capacity = 0; capacity < 5; ++capacity) {
        ...
        EXPECT_EQ(TLV_ERR_BUFFER_TOO_SHORT, tlv_write(data, capacity,
            &tlv_format_fixed_1byte, tag, value, sizeof(value), &written));
...
TEST(Writer, StatefulAppendFailureAndRetry) {
    ...
    tlv_reader_t reader;
    ASSERT_EQ(TLV_OK, tlv_reader_init(&reader, data, tlv_writer_size(&writer), &tlv_format_fixed_1byte));
    tlv_view_t view{};
...
TEST(Writer, ImplicitZeroByteLength) {
    ...
    tlv_view_t view{};
    ASSERT_EQ(TLV_OK, tlv_read(&data, size, &format, &view, &size));
    EXPECT_EQ(1u, size);
    EXPECT_EQ(tag.data[0], view.tag.data[0]);
    EXPECT_EQ(0u, view.value.length);
}
