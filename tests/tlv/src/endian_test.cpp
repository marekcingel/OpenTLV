...
TEST(TLVEndian, read_u16_be_from_exact_bytes) {
...
TEST(TLVEndian, write_u16_be_produces_exact_bytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
...
TEST(TLVEndian, read_u16_le_from_exact_bytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[3] = {};
        for (size_t i = 0; i < 2; ++i) {
...
TEST(TLVEndian, write_u16_le_produces_exact_bytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[4];
        for (auto& byte : storage) {
...
TEST(TLVEndian, read_u32_be_from_exact_bytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
...
TEST(TLVEndian, write_u32_be_produces_exact_bytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
...
TEST(TLVEndian, read_u32_le_from_exact_bytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[5] = {};
        for (size_t i = 0; i < 4; ++i) {
...
TEST(TLVEndian, write_u32_le_produces_exact_bytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[6];
        for (auto& byte : storage) {
