...
TEST(Unit_Tlv_Writer, StatefulAppendFailureAndRetry) {
    uint8_t       data[5] = {};
...
TEST(Unit_Tlv_Writer, CopyEncodedAppendsAtCurrentPositionAndAdvances) {
    uint8_t       data[8];
...
TEST(Unit_Tlv_Writer, CopyEncodedInsufficientCapacityLeavesPosition) {
    uint8_t       data[3];
    const uint8_t encoded[] = {0xAB, 0x02, 0x11, 0x22};
    tlv_writer_t  writer;
    ASSERT_EQ(TLV_OK, tlv_writer_init(&writer, data, sizeof(data), &controlled::writer));
...
TEST(Unit_Tlv_WriterDiagnostic, WriteDiagAcceptsANullOutParameter) {
...
TEST(Unit_Tlv_WriterDiagnostic, WriteDiagReportsRequiredSizeExceedingAvailableCapacity) {
...
TEST(Unit_Tlv_WriterDiagnostic, WriteDiagReportsAnUnsupportedTag) {
    uint8_t                 data[4] = {};
    size_t                  written = 0;
    tlv_writer_diagnostic_t diagnostic;
...
TEST(Unit_Tlv_WriterDiagnostic, WriteDiagReportsAnUnsupportedLengthWithTheValidatedTag) {
    const uint8_t           value[] = {0x11};
    uint8_t                 data[4] = {};
    size_t                  written = 0;
    tlv_writer_diagnostic_t diagnostic;
...
TEST(Unit_Tlv_WriterDiagnostic, InitResetsEveryFieldAndIgnoresANullDiagnostic) {
    tlv_writer_diagnostic_t diagnostic;
    std::memset(&diagnostic, 0xAA, sizeof(diagnostic));
    ...
    tlv_writer_diagnostic_init(nullptr);
}
