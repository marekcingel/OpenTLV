...
TEST(Unit_Tlv_Emv, ContextPreventsTagCollisions) {
...
TEST(Unit_Tlv_Emv, ChildContextTracksNestedTemplates) {
...
TEST(Unit_Tlv_Emv, SemanticErrorsAndUnalignedStorage) {
...
TEST(Unit_Tlv_Emv, AflCodecDecodesEntryList) {
...
TEST(Unit_Tlv_Emv, AflCodecRejectsMalformedEntries) {
    const auto*   codec = find(tlv_emv_tag_afl)->codec;
...
TEST(Unit_Tlv_Emv, Track2CodecRoundTripsWithAndWithoutPadding) {
...
TEST(Unit_Tlv_Emv, Track2CodecRejectsMalformedValues) {
    const auto*      codec = find(tlv_emv_tag_track2_equivalent_data)->codec;
...
TEST(Unit_Tlv_Emv, ValueKindDescriptionCoversEveryKind) {
...
TEST(Unit_Tlv_Emv, DisplayLabelAndTitlecaseName) {
...
TEST(Unit_Tlv_Emv, CoversContactBook3TagSet) {
...
TEST(Unit_Tlv_Emv, NumericConstantsInSwitch) {
