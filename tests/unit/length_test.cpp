...
TEST(Unit_Tlv_Length, FromSizeConvertsZeroAndMaximumLosslessly) {
...
TEST(Unit_Tlv_Length, FromSizeRejectsNullOutputAndLeavesItUnchanged) {
    EXPECT_EQ(TLV_ERR_NULL_ARG, tlv_length_from_size(0, nullptr));
}

TEST(Unit_Tlv_Length, ToSizeAcceptsZeroAndNativeMaximum) {
    size_t size = 99;
...
TEST(Unit_Tlv_Length, ToSizeRejectsNullOutput) {
    EXPECT_EQ(TLV_ERR_NULL_ARG, tlv_length_to_size(0, nullptr));
}

TEST(Unit_Tlv_Length, ToSizeRejectsLengthsBeyondSizeMaxAndLeavesOutputUnchanged) {
    size_t size = 99;
...
TEST(Unit_Tlv_Length, ValidateNativeAcceptsEverythingWithinSizeTRange) {
    EXPECT_EQ(TLV_OK, tlv_length_validate_native(0));
    EXPECT_EQ(TLV_OK, tlv_length_validate_native(std::numeric_limits<size_t>::max()));
}

TEST(Unit_Tlv_Length, ValidateNativeDoesNotAccessMemoryForOversizedLength) {
...
TEST(Unit_Tlv_Length, MaxIsTheFull64BitRange) {
    EXPECT_EQ(UINT64_MAX, static_cast<uint64_t>(TLV_LENGTH_MAX));
}

TEST(Unit_Tlv_Length, AddSumsWithinRange) {
    tlv_length_t sum = 99;
...
TEST(Unit_Tlv_Length, AddDetectsOverflowAndLeavesOutputUnchanged) {
    tlv_length_t sum = 99;
...
TEST(Unit_Tlv_Length, AddRejectsNullOutput) {
