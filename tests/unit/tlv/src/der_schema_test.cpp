...
TEST(Unit_DerSchema, DistinguishesSetFromSetOf) {
...
TEST(Unit_DerSchema, SetRequiresCanonicalTagOrder) {
...
TEST(Unit_DerSchema, SetOfRequiresCanonicalEncodingOrder) {
    tlv_der_schema_component_t  element = Required(kInteger);
    const tlv_der_schema_type_t set_of = {
        TLV_DER_SCHEMA_SET_OF, 0, nullptr, 0, &element, 0, SIZE_MAX};
...
TEST(Unit_DerSchema, SetOfElementCountBounds) {
    tlv_der_schema_component_t  element = Required(kInteger);
...
TEST(Unit_DerSchema, SequenceRequiredOptionalDefault) {
...
TEST(Unit_DerSchema, ImplicitTagging) {
...
TEST(Unit_DerSchema, ExplicitTagging) {
    tlv_der_schema_component_t component = {
...
TEST(Unit_DerSchema, ChoiceResolution) {
...
TEST(Unit_DerSchema, DepthLimitAppliesToPushingAFrame) {
...
TEST(Unit_DerSchema, ElementCountLimitAppliesAcrossNesting) {
    tlv_der_schema_component_t  component = Required(kInteger);
    const tlv_der_schema_type_t seq = {TLV_DER_SCHEMA_SEQUENCE, 0, &component, 1, nullptr, 0, 0};
    tlv_der_schema_limits_t     limits = tlv_der_schema_default_limits;
...
TEST(Unit_DerSchema, SchemaCheckCatchesAuthoringErrors) {
...
TEST(Unit_DerSchema, WriteSequenceOmitsDefaultEqualComponent) {
    static const uint8_t       kFalseDefault[] = {0x01, 0x01, 0x00};
...
TEST(Unit_DerSchema, WriteSequenceKeepsNonDefaultValue) {
    static const uint8_t       kFalseDefault[] = {0x01, 0x01, 0x00};
    tlv_der_schema_component_t components[2] = {
        Required(kInteger),
        {&kBoolean, TLV_DER_TAG_NONE, TLV_ASN1_UNIVERSAL, 0, TLV_DER_DEFAULT, kFalseDefault,
...
TEST(Unit_DerSchema, WriteSetOrdersComponentsByTag) {
...
TEST(Unit_DerSchema, WriteSetOfSortsElementsByCompleteEncoding) {
    tlv_der_schema_component_t  element = Required(kInteger);
    const tlv_der_schema_type_t set_of = {
        TLV_DER_SCHEMA_SET_OF, 0, nullptr, 0, &element, 0, SIZE_MAX};
...
TEST(Unit_DerSchema, WriteMissingRequiredFails) {
...
TEST(Unit_DerSchema, WriteRoundTripsThroughRead) {
    tlv_der_schema_component_t  components[2] = {Required(kOctetString), Required(kInteger)};
    const tlv_der_schema_type_t set_type = {TLV_DER_SCHEMA_SET, 0, components, 2, nullptr, 0, 0};
    std::vector<uint8_t>        output;
    ASSERT_EQ(TLV_OK, Write(set_type,
