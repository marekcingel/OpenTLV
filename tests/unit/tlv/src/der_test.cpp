...
TEST(Unit_Der, UniversalPrimitiveConstructedRules) {
    for (uint64_t number = 0; number <= 36; ++number) {
        ...
        for (int constructed : {0, 1}) {
            ...
            EXPECT_EQ(TLV_OK,
                      tlv_der_tag_make(TLV_ASN1_CONTEXT_SPECIFIC, constructed, number, &tag));
        }
    }
}
...
TEST(Unit_Der, EmptyArgumentsAndVisitorControl) {
    ...
    EXPECT_EQ(TLV_ERR_NULL_ARG,
              tlv_der_write(nullptr, 1, (tlv_tag_t{{4}, 1}), nullptr, 0, nullptr, &used, nullptr));
...
TEST(Unit_Der, TagSizeErrorsPreserveOutputs) {
    ...
    EXPECT_EQ(TLV_ERR_INVALID_TAG_SIZE, tlv_der_tag_make(TLV_ASN1_PRIVATE, 0, UINT64_MAX, &tag));
