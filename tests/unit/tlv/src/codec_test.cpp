...
tlv_codec_result_t decode_view(const void*, const uint8_t* data, size_t size, void* value,
                               size_t capacity) {
...
TEST(Unit_Codec, ScalarRoundTripAndSizeQuery) {
...
TEST(Unit_Codec, CallbackErrorsAndStorageBounds) {
    uint8_t  bytes[] = {0x12, 0x34};
    uint16_t value = 7;
    size_t   written = 99;
    EXPECT_EQ(tlv_codec_decode(&scalar, bytes, 1, &value, sizeof(value)),
              TLV_CODEC_ERR_INVALID_VALUE);
    EXPECT_EQ(tlv_codec_decode(&scalar, bytes, 2, &value, 1), TLV_CODEC_ERR_BUFFER_TOO_SHORT);
    EXPECT_EQ(value, 7);
    EXPECT_EQ(tlv_codec_encode(&scalar, &value, sizeof(value), bytes, 1, &written),
              TLV_CODEC_ERR_BUFFER_TOO_SHORT);
    EXPECT_EQ(written, 0u);
    EXPECT_EQ(bytes[0], 0x12);
    EXPECT_EQ(tlv_codec_encode(&scalar, &value, 0, nullptr, 0, &written),
              TLV_CODEC_ERR_INVALID_VALUE);
    EXPECT_EQ(written, 0u);
}

TEST(Unit_Codec, InvalidArgumentsAndUnsupportedDirections) {
    uint8_t           byte = 0;
    uint16_t          value = 0;
    size_t            written = 99;
    ...
    EXPECT_EQ(tlv_codec_encode(nullptr, &value, sizeof(value), &byte, 1, &written),
              TLV_CODEC_ERR_NULL_ARG);
    EXPECT_EQ(written, 0u);
    EXPECT_EQ(tlv_codec_encode(&scalar, nullptr, 0, &byte, 1, &written), TLV_CODEC_ERR_NULL_ARG);
    EXPECT_EQ(tlv_codec_encode(&scalar, &value, sizeof(value), nullptr, 1, &written),
              TLV_CODEC_ERR_NULL_ARG);
    EXPECT_EQ(tlv_codec_encode(&scalar, &value, sizeof(value), &byte, 1, nullptr),
              TLV_CODEC_ERR_NULL_ARG);
    EXPECT_EQ(tlv_codec_encode(&empty, &value, sizeof(value), nullptr, 0, &written),
