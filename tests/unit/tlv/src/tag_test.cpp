...
TEST(Unit_Tag, ByteArrayComparisonAcrossFullCapacity) {
    ...
    for (size_t i = 0; i < sizeof(bytes); ++i) tag.data[i] = bytes[i] = static_cast<uint8_t>(i);
...
TEST(Unit_Tag, CheckedU8Conversion) {
    check_narrow_conversion(tlv_tag_to_u8);
}
TEST(Unit_Tag, CheckedU16Conversion) {
    check_narrow_conversion(tlv_tag_to_u16);
}
TEST(Unit_Tag, CheckedU32Conversion) {
    check_narrow_conversion(tlv_tag_to_u32);
}
...
void check_narrow_equality(tlv_result_t (*compare)(const tlv_tag_t*, T, tlv_byte_order_t, int*)) {
    ...
    if (sizeof(T) <= TLV_TAG_CAPACITY) {
        tag.size = static_cast<uint8_t>(sizeof(T));
        std::memset(tag.data, 0xff, tag.size);
        check_comparison(TLV_OK, 1, compare, &tag, static_cast<T>(~T(0)),
...
TEST(Unit_Tag, U8Equality) {
    check_narrow_equality(tlv_tag_equal_u8);
}
TEST(Unit_Tag, U16Equality) {
    check_narrow_equality(tlv_tag_equal_u16);
}
TEST(Unit_Tag, U32Equality) {
    check_narrow_equality(tlv_tag_equal_u32);
}
TEST(Unit_Tag, U64Equality) {
    check_narrow_equality(tlv_tag_equal_u64);
}
...
TEST(Unit_Tag, U8ByteOrders) {
    check_byte_orders(tlv_tag_to_u8, tlv_tag_equal_u8);
}
TEST(Unit_Tag, U16ByteOrders) {
    check_byte_orders(tlv_tag_to_u16, tlv_tag_equal_u16);
}
TEST(Unit_Tag, U32ByteOrders) {
    check_byte_orders(tlv_tag_to_u32, tlv_tag_equal_u32);
}
TEST(Unit_Tag, U64ByteOrders) {
    ...
    check_comparison(TLV_OK, 1, tlv_tag_equal_u64, &tag, UINT64_C(0x0123456789abcdef),
    ...
    for (auto order : {TLV_BYTE_ORDER_BIG_ENDIAN, TLV_BYTE_ORDER_LITTLE_ENDIAN}) {
        for (size_t size = 1; size <= sizeof(uint64_t) && size <= TLV_TAG_CAPACITY; ++size) {
            ...
            for (size_t i = 0; i < TLV_TAG_CAPACITY; ++i)
                EXPECT_EQ(i == (order == TLV_BYTE_ORDER_BIG_ENDIAN ? size - 1 : 0) ? 0x82 : 0,
                          tag.data[i]);
            T value = 0;
...
TEST(Unit_Tag, ConstructU8) {
    check_constructor(tlv_tag_from_u8, tlv_tag_to_u8);
}
TEST(Unit_Tag, ConstructU16) {
    check_constructor(tlv_tag_from_u16, tlv_tag_to_u16);
}
TEST(Unit_Tag, ConstructU32) {
    check_constructor(tlv_tag_from_u32, tlv_tag_to_u32);
}
TEST(Unit_Tag, ConstructU64) {
    check_constructor(tlv_tag_from_u64, tlv_tag_to_u64);
}

TEST(Unit_Tag, IrregularNumericLengths) {
    const uint8_t  big[] = {1, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd};
    const uint64_t expected[] = {UINT64_C(0x012345), UINT64_C(0x0123456789),
                                 UINT64_C(0x0123456789abcd)};
    size_t         fixture = 0;
...
TEST(Unit_Tag, MultipleInvalidInputsKeepValidationOrder) {
    ...
    EXPECT_EQ(TLV_ERR_NULL_ARG, tlv_tag_from_u64(0, 0, TLV_BYTE_ORDER_UNKNOWN, nullptr));
...
TEST(Unit_Tag, ComparisonNullOutputsAndValidationOrder) {
    ...
    EXPECT_EQ(42, equal);
    EXPECT_EQ(TLV_ERR_NULL_ARG, tlv_tag_equal_u64(&tag, 0, TLV_BYTE_ORDER_UNKNOWN, nullptr));
    check_comparison(TLV_ERR_INVALID_TAG_SIZE, 0, tlv_tag_equal_u64, &tag, UINT64_MAX,
                     TLV_BYTE_ORDER_UNKNOWN);
    tag.size = 1;
    check_comparison(TLV_ERR_INVALID_BYTE_ORDER, 0, tlv_tag_equal_u64, &tag, UINT64_MAX,
                     TLV_BYTE_ORDER_UNKNOWN);
}
