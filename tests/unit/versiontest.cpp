...
TEST(Unit_Tlv_Version, ExposesBuildSystemVersion) {
    EXPECT_GE(OPENTLV_VERSION_REVISION, 0);

    const std::string core_version =
        OPENTLV_TEST_STRINGIFY(OPENTLV_VERSION_MAJOR) "." OPENTLV_TEST_STRINGIFY(
            OPENTLV_VERSION_MINOR) "." OPENTLV_TEST_STRINGIFY(OPENTLV_VERSION_PATCH);
    ...
    EXPECT_EQ(core_version + "-" + OPENTLV_TEST_STRINGIFY(OPENTLV_VERSION_REVISION) + "-" +
                  OPENTLV_GIT_COMMIT_HASH,
              OPENTLV_GIT_REPO_VERSION);
}

TEST(Unit_Tlv_Version, RuntimeApiReportsLoadedLibraryVersion) {
