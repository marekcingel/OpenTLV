...
TEST(Unit_TlvppDiagnostic, MakeDiagnosticPathStartsEmpty) {
    tlv::diagnostic_path path = tlv::make_diagnostic_path();

    EXPECT_EQ(0u, path.length);
}

TEST(Unit_TlvppDiagnostic, PushPathAppendsTagsInOrder) {
    tlv::diagnostic_path path = tlv::make_diagnostic_path();
...
TEST(Unit_TlvppDiagnostic, PushPathReturnsAnErrorWhenFull) {
    tlv::diagnostic_path path = tlv::make_diagnostic_path();
...
TEST(Unit_TlvppDiagnostic, PopPathRemovesTheLastTag) {
    tlv::diagnostic_path path = tlv::make_diagnostic_path();
...
TEST(Unit_TlvppDiagnostic, SetPathAttachesThePathToTheDiagnostic) {
    tlv::diagnostic diagnostic =
