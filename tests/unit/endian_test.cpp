...
TEST(Unit_Tlv_Endian, ReadU16BeFromExactBytes) {
...
TEST(Unit_Tlv_Endian, WriteU16BeProducesExactBytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
...
TEST(Unit_Tlv_Endian, ReadU16LeFromExactBytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[3] = {};
        for (size_t i = 0; i < 2; ++i) {
...
TEST(Unit_Tlv_Endian, WriteU16LeProducesExactBytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[4];
        for (auto& byte : storage) {
...
TEST(Unit_Tlv_Endian, ReadU32BeFromExactBytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
...
TEST(Unit_Tlv_Endian, WriteU32BeProducesExactBytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
...
TEST(Unit_Tlv_Endian, ReadU32LeFromExactBytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[5] = {};
        for (size_t i = 0; i < 4; ++i) {
...
TEST(Unit_Tlv_Endian, WriteU32LeProducesExactBytes) {
    for (const auto& test : cases) {
        SCOPED_TRACE(test.value);
        alignas(uint32_t) uint8_t storage[6];
        for (auto& byte : storage) {
...
TEST(Unit_Tlv_Endian, NativeOrderMatchesIntegerStorage) {
...
TEST(Unit_Tlv_Endian, CheckedWidthsExactBytesAndPadding) {
...
TEST(Unit_Tlv_Endian, CheckedFailuresPreserveOutputs) {
    uint8_t bytes[8];
    std::memset(bytes, 0xA5, sizeof(bytes));
    uint64_t value = 42;
...
TEST(Unit_Tlv_Endian, FixedU64ExactBytesUnaligned) {
