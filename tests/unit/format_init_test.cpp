...
TEST(Unit_Tlv_FormatInit, ReaderValidInputAndOptionalContext) {
...
TEST(Unit_Tlv_FormatInit, ReaderRejectsNullArgumentsWithoutModification) {
    const int           context = 42;
...
TEST(Unit_Tlv_FormatInit, WriterValidInputAndOptionalContext) {
    const int           context = 42;
    tlv_writer_format_t format{};
    for (const void* ctx :
         {static_cast<const void*>(&context), static_cast<const void*>(nullptr)}) {
        ASSERT_EQ(TLV_OK,
                  tlv_writer_format_init(&format, ctx, write_tag, write_length, length_size));
        EXPECT_EQ(ctx, format.context);
...
TEST(Unit_Tlv_FormatInit, WriterRejectsNullArgumentsWithoutModification) {
    const int           context = 42;
    ...
    for (int missing = 0; missing < 3; ++missing) {
        EXPECT_EQ(TLV_ERR_INVALID_ARG,
                  tlv_writer_format_init(&format, nullptr, missing == 0 ? nullptr : write_tag,
