...
| `TLV_ERR_INVALID_LENGTH` | 2 | A length is malformed, out of range, or does not fit `size_t`. | The format's length limits (for example 255 for [Fixed 1-byte TLV](../formats/fixed/README.md), 65,535 for [Default TLV](../formats/default/README.md)), the reserved BER length prefix `FF`, or a total that overflows `size_t`. |
...
| `TLV_ERR_INVALID_TAG_SIZE` | 11 | A tag size is outside the range the operation supports. | Empty tags, and tags larger than `TLV_TAG_CAPACITY` (8 bytes by default). |
| `TLV_ERR_INVALID_BYTE_ORDER` | 12 | A byte order is unknown or unsupported. | The `TLV_BYTE_ORDER_*` value passed to the integer conversion functions in `tlv/endian.h` and `tlv/tag.h`. |
