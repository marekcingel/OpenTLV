...
universal types, `SEQUENCE`, `SET`, `SET OF`, `CHOICE`, `ANY`, IMPLICIT/EXPLICIT
tagging, and `OPTIONAL`/`DEFAULT` components. It is not an ASN.1 compiler. Every ASN.1
profile below depends on how much of X.680 these rows cover.

| Candidate | Specification | Subset | Components | Reuse and limits | Check |
| --- | --- | --- | --- | --- | --- |
| Wider X.680 type coverage (`[?]`) | X.680 | Constructs not in the current schema subset: `SEQUENCE OF`, `AUTOMATIC TAGS`, extension markers with unknown additions, `SIZE` and value-range constraints, named numbers and bits, `ENUMERATED` values, `COMPONENTS OF`. Also the universal types that strict mode rejects today (ObjectDescriptor, TeletexString, VideotexString, GraphicString, GeneralString, TIME, DATE, TIME-OF-DAY, DATE-TIME, DURATION, OID-IRI, RELATIVE-OID-IRI). | DER schema, value codecs | Extends `tlv_der_schema_type_t`. Must stay static, caller-owned tables without allocation. New kinds change the schema API and need a compatibility review. Constraint checking is value semantics beyond framing. | Not verified: ITU-T recommendation not read. Current coverage read from the [DER profile](../profiles/der/README.md). |
