...
is under active development, and further bindings may follow. Without a
shared contract, each binding could invent its own vocabulary for the same
...
C++ and Rust ship today; Go and Python are illustrative here, not yet
scaffolded, to show the contract extends past the two current bindings (Go is
also planned; Python is the example from the original design request). The
