...
tlv_result_t tlv_ber_write_indefinite(uint8_t* data, size_t capacity, tlv_tag_t tag,
    ...
    if (rc != TLV_OK) return rc;
    // NOLINTBEGIN(clang-analyzer-unix.cstring.NullArg,clang-analyzer-core.NullDereference):
    ...
    // across the indirect write_tag() call inside tlv_ber_indefinite_encoded_size().
    memcpy(data, tag.data, tag.size);
    data[tag.size] = 0x80;
    // NOLINTEND(clang-analyzer-unix.cstring.NullArg,clang-analyzer-core.NullDereference)
